...
- 历史审计日志不做无边界删除或重写：已确认采用受限保留策略，保留现有
  `640 root:systemd-journal` 权限并将旧 token 失效作为凭据事件处置结果；当前
  journald boot 还包含其他审计记录，后续如需脱敏须按组织批准的范围执行。

## P0 关单判断

代码、生产部署、凭据轮换、旧 token 失效验证和日志访问限制均已完成；历史日志按
受限保留策略留存，不再作为 P0 阻塞项。后续 P1 继续遵循“先修复、再独立验收、再发布”的门槛。
