...
def _require_merchant_control(db_path: str | Path, merchant_id: str, payload: dict[str, Any]):
    """校验调用方控制该商家（owner token 或 admin token）。"""
    try:
        api_auth.require_admin_token(payload)
    except api_auth.AuthError:
        api_auth.require_merchant_token(payload, merchant_id)
...
def bind_shopping_token(
    ...
    merchant_id = _require_merchant_control(db_path, merchant_id, payload)
